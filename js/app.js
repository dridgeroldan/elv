...
document.getElementById('kpiForm').addEventListener('submit', async e => {
  ...
  // Delete existing entries for this employee+date, then re-insert
  ...
  // Compute daily avg and update/create monthly summary
  const totalWV = entries.reduce((sum, en) => sum + (en.weighted_value || 0), 0);
  const monthYear = date.slice(0, 7);

  // Get all entries for this emp+month to recalc avg
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Monthly filters
  document.getElementById('monthlyEmpFilter').addEventListener('change', async () => {
    renderMonthlyGrid(employees, allSummaries);
  });
  document.getElementById('monthlyMonthFilter').addEventListener('change', async () => {
    renderMonthlyGrid(employees, allSummaries);
  });
